...
class FilesProviderTests: XCTestCase, FileProviderDelegate {
    ...
    func testDropbox() {
        ...
        let cred: URLCredential?
        if let user = ProcessInfo.processInfo.environment["dropbox_user"], let pass = ProcessInfo.processInfo.environment["dropbox_password"] {
            cred = URLCredential(user: user, password: pass, persistence: .forSession)
        } else {
            cred = nil
        }
    ...
    func testOneDrive() {
        let cred: URLCredential?
        if let user = ProcessInfo.processInfo.environment["onedrive_user"], let pass = ProcessInfo.processInfo.environment["onedrive_token"] {
            cred = URLCredential(user: user, password: pass, persistence: .forSession)
        } else {
            cred = nil
        }
